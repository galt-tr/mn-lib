...
func CreateOpPushTxWithChange(utxos []*bsv.Utxo, mnAddress *PayToMetanetAddress, changeAddress string, standardRate, dataRate *bt.Fee, privateKey *bsvec.PrivateKey) (*bt.Tx, error) {
	...
	// Create the "Draft tx"
	fee, err := draftTx(utxos, mnAddress, "", privateKey, standardRate, dataRate)
	if err != nil {
		return nil, err
	}
	...
	if (totalPayToSatoshis + fee) > totalSatoshis {
		...
		if fee, err = draftTx(utxos, mnAddress, "", privateKey, standardRate, dataRate); err != nil {
			return nil, err
		}
		mnAddress.HasChange = true
	...
	return CreateOpPushTx(utxos, mnAddress, privateKey)

}

func CreateOpPushTx(utxos []*bsv.Utxo, mnAddress *PayToMetanetAddress, privateKey *bsvec.PrivateKey) (*bt.Tx, error) {
	...
	if s, err = script.AppendP2PKHLockingScript(s, mnAddress.Address); err != nil {
		return nil, err
	}
	...
	//	return nil, err
	//}
	fmt.Println(tx.Inputs[0].UnlockingScript.ToASM())
	fmt.Println()
...
func draftTx(utxos []*bsv.Utxo, payTo *PayToMetanetAddress, data string, privateKey *bsvec.PrivateKey, standardRate, dataRate *bt.Fee) (uint64, error) {
	...
	//	return 0, err
	//}
	tx, err := CreateOpPushTx(utxos, payTo, privateKey)
	if err != nil {
		return 0, err
	}
